...
export class NetscriptExtension {
  ...
  calculateStockStats(): {
    ...
    this.ns.stock.getSymbols().forEach((stockSymbol) => {
      ...
      if (sharesLong > 0) {
        const longSharesProfit =
          sharesLong * (this.ns.stock.getBidPrice(stockSymbol) - avgLongPrice) - 2 * STOCK_MARKET_COMMISSION_FEE;
      ...
      if (sharesShort > 0) {
        const shortSharesProfit =
          sharesShort * (avgShortPrice - this.ns.stock.getAskPrice(stockSymbol)) - 2 * STOCK_MARKET_COMMISSION_FEE;
    ...
    const estimatedTotalProfit =
      this.ns.getPlayer().money + currentWorth - this.ns.getMoneySources().sinceInstall.hacking + privateServersCost;
    return {
