 ...
 */
import type { Endpoint, EventSource, PostMessageWithOrigin } from './protocol';
...
/** Stand-in for `Function` in distributive property types (ESLint bans `Function`). */
type ComlinkFunctionLike = ((...args: never[]) => unknown) | (new (...args: never[]) => unknown);
/**
 ...
 */
type RemoteProperty<T> = T extends ProxyMarked | ComlinkFunctionLike ? Remote<T> : Promisify<T>;
/**
 ...
 */
type LocalProperty<T> = T extends ProxyMarked | ComlinkFunctionLike ? Local<T> : Unpromisify<T>;
/**
...
export declare function expose(obj: object, ep?: Endpoint, allowedOrigins?: (string | RegExp)[]): void;
export declare function wrap<T>(ep: Endpoint, target?: unknown): Remote<T>;
...
export declare function windowEndpoint(
