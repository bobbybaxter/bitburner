...
import { log } from './log';
import type { ArgsSchemaEntry } from './types';
...
export function getConfiguration(ns: NS, argsSchema: ArgsSchemaEntry[]): Record<string, unknown> | null {
  ...
  if (overrides) {
    try {
      ...
      const schemaIndexByName = new Map(overriddenSchema.map((entry, i) => [entry[0], i]));
      for (const key in parsedOverrides) {
        const override = parsedOverrides[key];
        const matchIndex = schemaIndexByName.get(key) ?? -1;
