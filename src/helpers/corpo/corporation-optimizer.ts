...
import { CityName, CorpIndustryData, Division, Material, Product } from '@ns';
import { CorpMaterialsData, CorpUpgradesData } from '/constants/corp';
import {
...
import { PriorityQueue } from '/helpers/corpo/priority-queue.js';
import { scaleValueToRange } from '/helpers/corpo/scale-value-to-range.js';
    ...
    if (!useCurrentItemData) {
      ...
      demand =
        division.awareness === 0
          ? 20
          : Math.min(100, advertisingInvestmentMultiplier * (100 * (division.popularity / division.awareness)));
      // Hard-coded value of getRandomInt(0, 70). We don't want RNG here.
      competition = 35;
    } else {
      productEffectiveRating = item.effectiveRating;
      productMarkup = await getProductMarkup(division, industryData, 'Sector-12' as CityName, item, undefined);
    ...
    for (let wilsonLevel = currentWilsonLevel; wilsonLevel <= maxWilsonLevel; wilsonLevel++) {
      ...
      for (let advertLevel = currentAdvertLevel + 1; advertLevel <= maxAdvertLevel; advertLevel++) {
        ...
        const advertisingMultiplier =
          (1 + CorpUpgradesData[UpgradeName.WILSON_ANALYTICS].benefit * wilsonLevel) * researchAdvertisingMultiplier;
