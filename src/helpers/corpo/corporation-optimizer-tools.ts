...
import type { Remote } from '../../libs/comlink.d.ts';
import * as comlink from '../../libs/comlink.js';
import { NetscriptExtension } from '../../libs/NetscriptExtension';
import { ScriptFilePath } from '../../libs/paths/ScriptFilePath';
import { generateBlobUrl } from '../../libs/script-utils.js';
import { Do } from '../do.js';
import { calculateEmployeeStats, formatNumber, ResearchName } from './corporation-formulas';
import {
...
async function splitWorkload(
  ...
  for (let i = 0; i < numberOfThreads; ++i) {
    ...
    workers.push(worker);
    const workerWrapper = comlink.wrap(worker) as Remote<CorporationOptimizer>;
    promises.push(
      workload(
        worker,
        workerWrapper,
...
export async function optimizeOffice(
  ...
  const office = (await Do(nsx.ns, 'ns.corporation.getOffice', division.name, city)) as ReturnType<
    NS['corporation']['getOffice']
  >;

  let avgMorale = office.avgMorale;
  let avgEnergy = office.avgEnergy;
  const corporationUpgradeLevels = await getCorporationUpgradeLevels(nsx.ns);
  const divisionResearches = await getDivisionResearches(nsx.ns, division.name);
  ...
  const printDataEntryLog = (dataEntry: OfficeBenchmarkData) => {
    let message =
      `{operations:${dataEntry.operations}, engineer:${dataEntry.engineer}, ` +
