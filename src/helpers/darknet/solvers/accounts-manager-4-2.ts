...
const PROGRESS_FILE = '/helpers/darknet/accounts-manager-4-2-progress.json';
...
export async function solveAccountsManager42(ns: NS, hostname: DarknetHostname): Promise<DarknetSolverResult> {
  ...
  const stop = Math.min(maxValue, start + GUESSES_PER_PASS);
