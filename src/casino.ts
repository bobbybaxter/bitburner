...
/** Inlined Bitburner WHRNG — avoids allocating ~30M objects during seed search. */
function rouletteStreamMatchesTotalPlaytime(totalPlaytime: number, spins: readonly number[]): boolean {
  ...
  for (const spin of spins) {
    ...
    s2 = (172 * s2) % 30307;
    s3 = (170 * s3) % 30323;
    const r = (s1 / 30269.0 + s2 / 30307.0 + s3 / 30323.0) % 1.0;
    if (spin !== Math.floor(r * 37)) {
      return false;
    }
  }
  return true;
...
<div id="casino-tools">
  ...
  casinoToolsDiv.querySelector('#btn-guess-seed')!.addEventListener('click', () => {
    ...
    // Roulette seeds WHRNG once at UI mount with Date.now(), not when you click Guess Seed — so the
    // matching offset in [0, maxSeed) is not necessarily near (guessTime - zeroDate). Scan 0..maxSeed.
    let possibleSeed = 0;
    while (possibleSeed < maxSeed) {
      if (rouletteStreamMatchesTotalPlaytime(zeroDate + possibleSeed, spinsForGuessing)) {
        rouletteSeedElement.value = (zeroDate + possibleSeed).toString();
        break;
      }
