...
- `hack.js` - manually hack a target
- `grow.js` - manually grow a target
- `weak.js` - manually weaken a target

## level 1 scripts

- `find-optimal-target.js` - finds the best target to hack at the moment
- `hack-lvl-1-service.js` - runs a basic service to hack the optimal target
  - can supply a dedicated host or target (ex. `run hack-lvl-1-service.js home n00dles`)
